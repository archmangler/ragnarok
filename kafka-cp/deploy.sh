...
function create_topics () {
  ...
  do
    kubectl -n kafka exec kafka-client -- kafka-topics --zookeeper kafka-cp-zookeeper-headless:2181 --topic messages --create --partitions 3 --replication-factor 1 --if-not-exists
  done
}

function check_kafka_topic_health () {
    printf "\n"
    OUT=$(kubectl -n kafka exec testclient -- kafka-topics --zookeeper kafka-cp-zookeeper-headless:2181 --describe)
    printf "$OUT\n"
}
